...
**M4 · 生产化**（任务 T-401 起，计划见 [docs/dev-plan-m4.md](docs/dev-plan-m4.md)）。
M0–M3 功能面已落地（~15k 行、166 测试、全生产驱动双副本实测跑通），但尚不能上企业生产：
差距在安全边界（沙箱未容器级断网）、真实模型能力（基线 40%，要 85%）、日常可用性（令牌 5 分钟过期无续期）、
运维成熟度（无指标/告警）。**先做 P0，不做 PRD 之外的顺手优化。**
