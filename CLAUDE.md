...
**M4 · 生产化，工程侧 18/23 完成**（2026-09-07，计划与进度见 [docs/dev-plan-m4.md](docs/dev-plan-m4.md)）。
沙箱已容器级断网（出网经 server 中继）、令牌静默续期、审计 DB 级不可变、主密钥可轮换、指标告警、策略中心、
API Key、多租户、前端 e2e 进 CI。**仍不能宣布生产可用**：T-416 真实模型 ≥85%（需 GPU）是硬门，
T-421–423（预发/渗透/上线）需客户环境。**接手者先看 docs/production-readiness.md 的补跑清单。**
