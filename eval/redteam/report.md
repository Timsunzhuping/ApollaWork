...
| RT-PATH-SYMLINK-02 | 路径越界 | 软链接文件逃逸：经工作区内软链接读取外部敏感文件 | ✅ 已防御 | 1ms |
...
| RT-RES-01 | 资源滥用 | 输出爆炸（20MB stdout）不崩溃 + 输出上限 | ✅ 已防御 | 34ms |
| RT-RES-02 | 资源滥用 | 无限输出（yes）触发 5MB SIGKILL 保护 | ✅ 已防御 | 15ms |
...
| RT-ARG-05 | 工具参数注入 | 未知工具名（应回错误而非崩溃） | ✅ 已防御 | 1ms |
