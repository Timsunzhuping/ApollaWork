...
- [ ] **T-301 技能/连接器市场**：上架、签名校验、审核流、版本管理。
- [ ] **T-302 桌面壳**：Electron + 本地执行模式（复用 protocol 事件与 UI；本地沙箱用 @anthropic-ai/sandbox-runtime，Apache-2.0）。
- [ ] **T-303 评测-微调闭环**：失败样本采集 → 标注 → 蒸馏/微调管线对接。
