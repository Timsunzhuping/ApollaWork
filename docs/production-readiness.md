# 生产就绪状态

> 快照：2026-08-28。对应本轮 P0/P1/P2 治理。上线操作见 [production.md](production.md)。

## P0 · 上线阻断项（全部关闭）

| # | 问题 | 处理 | 验证方式 |
|---|---|---|---|
| 1 | **水平越权**：任意登录用户可读写他人任务/文件/审批/SSE 流 | 新增 `AccessService`，一切资源先解析到 workspace 再判权限；非成员 404、越权 403 | 18 项越权单测（跨组织、非成员、viewer 写、editor 删空间…）+ 线上探测 404 |
| 2 | **垂直越权**：普通成员可改模型密钥、读全量审计 | admin/connectors/marketplace 端点全部要求组织管理员 | 单测 + 端点核查 |
| 3 | **生产静默丢数据**：compose.prod 配 `s3` 但代码只有 fs 驱动 | 实现 S3/MinIO 驱动 + materialize/persist 模型 | 7 项单测 + **真实 MinIO 端到端**（上传→执行→产物写回，对象确在 MinIO） |
| 4 | **沙箱隔离未生效**：镜像未构建、DockerExecutor 零验证 | 修 Dockerfile 符号链接缺陷（多阶段 hoisted）+ 构建期自检；修 DockerExecutor **审批/提问从不下发**（ask 模式生产不可用） | 见下方「待完成」 |
| 5 | **无登录界面**：oidc 模式下应用打不开 | 前端 OIDC 授权码+PKCE 登录页；SSE/下载走查询参数令牌 | 编译通过、认证配置端点已验证 |
| 6 | **IM 回调无验签**：公网暴露即未授权 RCE | 三通道真实验签，安全默认 fail-closed | 38 测试，含腾讯官方测试向量互操作 + 变异测试 |
| 7 | **DB 路径二义性**：CLI 与应用解析到不同库 | 统一解析 + 启动期表结构校验 | 实测复现并修复 |

## P1 · 生产可用性（全部关闭）

| # | 问题 | 处理 | 验证 |
|---|---|---|---|
| 8 | 进程内事件总线 → 多副本下用户看不到进度 | Redis Pub/Sub 跨副本分发，不可用时自动退化 | 6 项跨副本测试 |
| 9 | **并发缺陷**：各副本本地 seq 计数会撞号 | Redis INCR 原子分配（从 DB 预热） | 测试捕获并验证修复 |
| 10 | BullMQ 从未接线 | 队列抽象 + BullMQ 实现（jobId 幂等、崩溃可接管） | 编译 + 抽象隔离 |
| 11 | 重启后任务永久卡 running | 启动恢复收尾为 failed 并补事件 | 实测 |
| 12 | 无 DB 迁移（db push） | 基线迁移 + PG 迁移 | **真实 PostgreSQL 16 建库+种子+端到端任务** |
| 13 | 无健康/就绪探针 | `/healthz` `/readyz`（免鉴权，含 DB/存储探测） | 实测 |
| 14 | 配额只记录不拦截 | 组织/个人月度 token 上限，创建前拦截 | 实现 + 用量按人归集 |

## P2 · 运维与合规（全部关闭）

| # | 项 | 验证 |
|---|---|---|
| 15 | CI 三流水线（构建/测试/黄金/红队严格/性能 + 依赖漏洞 + 许可合规） | 各步骤本机实跑退出码 0；**未在真实 Actions runner 跑过** |
| 16 | 备份恢复三件套 | **备份→销毁数据→恢复→逐条比对** 端到端真跑；篡改检测、幂等、保留策略 |
| 17 | 安全响应头 + 限流 + CORS 白名单 | 实测：CSP/X-Frame 等头存在；限流 429 生效；健康探针豁免 |
| 18 | 结构化日志 + requestId 贯穿 | 实测 x-request-id 回传 |
| 19 | i18n（中/英） | 语言包 + 切换入口 |
| 20 | **生产就绪门**：带开发默认值上生产直接拒绝启动 | 实测拦截 4 项并给出修复命令；豁免开关可放行 |

## 待完成（诚实标注）

- **沙箱镜像与 DockerExecutor 端到端**：Dockerfile 已修正并加构建期自检，镜像构建耗时较长（ubuntu + LibreOffice + Python 栈）。
  在镜像构建完成并跑通一个 `EXECUTOR=docker` 任务前，**不应认为 P0#4 已关闭**。
- **未在真实环境验证**：GitHub Actions runner、K8s 集群 apply、Keycloak 真实登录跳转、IM 真实收发、gVisor。
  这些都已交付可运行代码/配置，缺的是运行环境。
- **Redis 真实互通**：跨副本逻辑用符合 ioredis 语义的假实现做了测试；真实 Valkey/Redis 容器因镜像拉取受阻未跑通。

## 测试总览

| 套件 | 数量 | 说明 |
|---|---|---|
| 单元/集成 | 130 | protocol 7 / agent-tools 15 / runtime 9 / server 48 / im-bridge 38 / desktop 13 |
| 黄金场景 | 10/10 | 发布闸门 |
| 安全红队 | 48/48 | 严格模式零缺口（6 类攻击面） |
| 性能 | 达标 | 20/50 并发均满足 PRD §1.6 |
