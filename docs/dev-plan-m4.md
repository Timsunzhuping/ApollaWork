...
| 状态 | 任务 |
|---|---|
| ✅ 完成（工程侧） | T-401 基线校正 · T-402 沙箱无网 + 出网中继 · T-403 容器加固 · T-404 上传治理 · T-405 审计不可变 · T-406 主密钥轮换 · T-407 日志脱敏 · T-408 令牌续期 · T-409 模型韧性 · T-410 成员管理 · T-411 前端测试基建 + e2e · T-412 指标告警 · T-413 策略中心 · T-414 Helm 生产化 + ADR-006 · T-415 加密/TLS + 恢复演练 · T-418 多租户 + 迁移 · T-419 API Key · T-420 失败样本 |
| 🟡 决策已定待确认 | T-417 按乙推进（关键词 v1，向量 v2）—— 待产品确认 |
| ⛔ 阻塞于环境 | T-416 真实模型评测（GPU）· T-421 预发内测 · T-422 渗透 · T-423 上线 |

**本机无法完成的验证**（Docker Desktop 等管理员授权弹窗，我不能替你输密码）：T-402/403 真容器回归、
T-405 真 PG 触发器、T-408 Keycloak 60s 令牌 e2e、T-415 恢复演练实跑。授权后各自一条命令即可补跑，
见 [production-readiness.md](production-readiness.md)。

**执行中额外抓到并修复的 5 个真 bug**（都不在计划里，都是做 e2e 才暴露的）：
实时事件乱序丢失（SSE 中间事件消失）、界面所有 DELETE 静默 400、本地 SQLite 已不可用（P1012）、
IM bridge 不带认证头（OIDC 下全 401）、`SKILLS_VOLUME` 检查不可满足逼开全局豁免。
